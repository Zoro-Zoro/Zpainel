...
def ip():
	n1 = str(input(f'{vm}--={f}{az}[{cn}CONSULTA IP{az}]{f}\n\n- {az}[{f}{cn}01{f}{az}] {br}ipgeolocation{f}\n- {az}[{f}{cn}02{f}{az}] {br}ip{f}\n- {az}[{f}{cn}03{f}{az}] {br}ipfind{f} \n- {am}+=>{f} '))#escolher uma das opções de api
	if n1=='1' or n1=='01':
		ip1=input(f'{vm}--=={f}{az}[{cn}fonte IPGEOLOCATION{az}]{f} \n{vm}-=={f}{az}[{cn} ip {az}] {am}+=>{f}')#A f9nte de consulta dessa é a ipgeolocation
		url1 = "https://api.ipgeolocation.io/ipgeo?apiKey=be35b8be10834da8af67daff92154013&ip={}".format(ip1)#api ipgeolocation
		res = requests.get(url1);req=res.json()#fazendo a requisição get e transformando a requisição para json
		b=f'\n{vm}-={az}[{cn}+{az}] {br}CONSULTA FEITA PELO PROGRAMA Zp4in3l\n{vm}-={az}[{cn}+{az}] {br}by: Zorrinho\n{vm}-={az}[{cn}ip{az}] {br}{req["ip"]}\n{vm}-={az}[{cn}country{az}] {br}{req["country_name"]}\n{vm}-={az}[{cn}estado{az}] {br}{req["state_prov"]}\n{vm}-={az}[{cn}city{az}] {br}{req["city"]}\n{vm}-={az}[{cn}latitude{az}] {br}{req["latitude"]}\n{vm}-={az}[{cn}longitude{az}] {br}{req["longitude"]}\n{vm}-={az}[{cn}time zone{az}] {br}{req["time_zone"]}\n{vm}-={az}[{cn}isp{az}] {br}{req["isp"]}\n{vm}-={az}[{cn}currency{az}] {br}{req["currency"]}\n{vm}-={az}[{cn}country flag{az}] {br}{req["country_flag"]}'#uma variavel criada para tratar os dados api
		print(b)#printando a variavel que foi usada para tratar os dados
		exit()#saindo do programa porque o script nao foi feito para voltar ao menu quando uma consulta for terminada
		#isso vale para todas as consultas.
...
def mip():
	...
	res=requests.get(url1);req1=res.text
	b9=f'\n{vm}-={az}[{cn}+{az}]{br}CONSULTA FEITA PELO PROGRAMA Zp4in3l \n{vm}-={az}[{cn}+{az}]{br}by: Zorrinho \n{vm}-={az}[{cn}IP{az}]{br}{req1}'
	print(b9)
...
def cpfg():
	...
	res=requests.post(url,  {"acao":"gerar_cpf", "pontuacao":"S"});req=res.text
	b2 = f'{br}[{vd}CPF{br}] {req}'
	print(b2)


def cnpjg():
	url = 'https://www.4devs.com.br/ferramentas_online.php'
	res=requests.post(url,  {"acao":"gerar_cnpj", "pontuacao":"S"});req=res.text
	b2 = f'{br}[{vd}CNPJ{br}] {req}'
	print(b2)


def contag():
	url = 'https://www.4devs.com.br/ferramentas_online.php'
	res=requests.post(url,  {"acao":"gerar_conta_bancaria", "banco":"", "estado":""});req=res.text
	b6 = f'{br}[{vd}RESULT{br}]\n{req}'
	print(b2)

def nomesg():
	url = 'https://www.4devs.com.br/ferramentas_online.php'
	res=requests.post(url,  {"acao":"gerar_nome", "raca":"orc", "sexo":"M"});req=res.text
	b7 = f'{br}[{vd}RESULT{br}]\n{req}'
	print(b7)

def nickg():
	url = 'https://www.4devs.com.br/ferramentas_online.php'
	res=requests.post(url,  {"acao":"gerar_nicks", "raca":"orc", "sexo":"M"});req=res.text
	b3 = f'{br}[{vd}RESULT{br}]\n{req}'
	print(b3)

def rgg():
	url = 'https://www.4devs.com.br/ferramentas_online.php'
	res=requests.post(url,  {"acao":"gerar_rg", "pontuacao":"S"});req=res.text
	b8 = f'{br}[{vd}RG{br}] {req}'
	print(b8)
	...
	print(f'''{cn}
	  ...
	  `  |/  V  V  `   V  \#\| |  | |/#/  V   '  V  V  \|  '
	     `   `  `      `   / | |  | | \   '      '  '   '
	                     ...
	                     __\ | |  | | /__
	{l}
	{lv}	{br}[{vd} consultas {br}]      {lv}	  {br}[{vd} geradores {br}]     {lv}
	{lv} {az}[{cn}01{az}] {br}ip                  {lv} {az}[{cn}11{az}] {br}gerar cpf         {lv}
	{lv} {az}[{cn}02{az}] {br}cep                 {lv} {az}[{cn}12{az}] {br}gerar cnpj        {lv}
	{lv} {az}[{cn}03{az}] {br}cnpj                {lv} {az}[{cn}13{az}] {br}conta bancaria    {lv}
	{lv} {az}[{cn}04{az}] {br}cpf                 {lv} {az}[{cn}14{az}] {br}gerar nomes       {lv}
	{lv} {az}[{cn}05{az}] {br}numero              {lv} {az}[{cn}15{az}] {br}gerar nicks{br}[{vm}off{br}]  {lv}
	{lv} {az}[{cn}06{az}] {br}localidade          {lv} {az}[{cn}16{az}] {br}gerar veiculo{br}[{vm}off{br}]{lv}
	{lv} {az}[{cn}07{az}] {br}nome                {lv} {az}[{cn}17{az}] {br}gerar rg          {lv}
	{lv} {az}[{cn}08{az}] {br}covid               {lv}{l3}{lv}
	{lv} {az}[{cn}09{az}] {br}meu ip              {lv}      {az}[{cn}99{az}]{br}criador       {lv}
	{lv} {az}[{cn}10{az}] {br}Link WhatSapp       {lv}      {az}[{cn}00{az}]{br}Sair          {lv}
	{l}
	''')
...
try:
	if inpu=='1' or inpu=='01':
		...
		clear();ban();time.sleep(6);os.system('termux-open-url https://youtube.com/channel/UCTRSuQ_j0qwRKxI9mMioeyg');looping()
	
	elif inpu=='0' or inpu=='00':
		print(f'{vm}-={az}[{cn}!!{az}] {br}Obrigado por escolher o Zp4in3l :)');exit()
